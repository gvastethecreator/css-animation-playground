...
export const easingFunctions: Record<EasingName, (t: number) => number> & { createBezier: typeof createBezier } = {
    ...
    easeInOutBounce: (x) => x < 0.5 ? (1 - easingFunctions.easeOutBounce(1 - 2 * x)) / 2 : (1 + easingFunctions.easeOutBounce(2 * x - 1)) / 2,
 ...
 */
export const getEasingFunction = (easing: EasingName | string): ((t: number) => number) => {
  ...
  if (typeof easing === 'string' && easing.startsWith('cubic-bezier')) {
      try {
          ...
          console.warn("Failed to parse cubic-bezier string:", easing, e);
      }
  }
