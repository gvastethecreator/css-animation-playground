...
const EngineSelector: React.FC<EngineSelectorProps> = ({ engine: currentEngine, onEngineChange, disabled }) => {
  return (
    <div
      ...
      {(Object.keys(ENGINE_COLORS) as AnimationEngine[]).map((engine) => {
        ...
        return (
          <button
            ...
            disabled={disabled}
            className={`relative flex-1 text-center text-[11px] font-bold uppercase tracking-wider px-2 py-1.5 rounded-md transition-all ${isActive ? `${colors.bg} ${colors.text}` : 'text-zinc-500 hover:bg-zinc-800'
              }`}
          >
            {label}
            {isActive && <div className="absolute bottom-1 left-1/2 -translate-x-1/2 w-1 h-1 bg-white rounded-full" />}
          </button>
        );
      })}
    </div>
