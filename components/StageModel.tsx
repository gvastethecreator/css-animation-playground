...
const StageModel = React.forwardRef<HTMLDivElement, StageModelProps>(({ transforms, isAdjusting, isPlaying, modelDataUrl, onFileChange, showStageUI, willChangeString, isExploded, onClick }, ref) => {
    const { isDraggingOver, handleDragEnter, handleDragLeave, handleDragOver, handleDrop } = useStageDragHandlers(onFileChange);
        ...
        >
            {modelDataUrl ? (
                <div className="w-80 h-80 rounded-xl border-2 border-dashed border-zinc-700 bg-zinc-900/50 flex flex-col items-center justify-center text-center p-4">
                    ...
                    <p className="mt-1 text-sm text-zinc-600">This element is only visible with the <b className="text-zinc-400">Three.js</b> engine. Please select it from the header.</p>
                </div>
            ) : (
                <label
                ...
                >
                    ...
                    <input id="stage-file-upload-model" type="file" className="hidden" onChange={(e) => e.target.files && onFileChange(e.target.files[0])} accept=".gltf,.glb" />
                </label>
            )}
        </StageLayers>
    );
