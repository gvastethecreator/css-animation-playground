    ...
    return (
        ...
        >
            <Tooltip content={isOpen ? 'Collapse' : 'Expand'}>
              ...
              >
                  {isOpen ? <ChevronDown size={16} className="mr-2 opacity-70" /> : <ChevronRight size={16} className="mr-2 opacity-70" />}
                  <h3 className="font-bold text-zinc-300 uppercase tracking-wider text-[12px]">{title}</h3>
              </button>
            </Tooltip>
            {isOpen && (
                <div className="px-3 pb-3 pt-2 space-y-3 border-t border-zinc-800">
                    {children}
                </div>
            )}
        </div>
