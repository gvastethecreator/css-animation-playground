...
const TimelineControls: React.FC<TimelineControlsProps> = ({
    ...
    const toggleDirection = () =>
        onTimelineStateChange((prev) => ({ ...prev, direction: prev.direction === 'normal' ? 'alternate' : 'normal' }));
    ...
    const prevFrame = () =>
        onTimelineStateChange((prev) => ({ ...prev, currentTime: Math.max(0, prev.currentTime - 1000 / prev.fps) }));
        ...
        <div className="flex items-center gap-3">
            <div className="flex items-center gap-0.5 bg-zinc-900 rounded-lg p-0.5 border border-zinc-800">
                <Tooltip content="Rewind (Home)">
                    <button onClick={rewind} className={iconBtnClass}>
                        <Rewind size={14} strokeWidth={2} />
                    </button>
                </Tooltip>
                <Tooltip content="Previous Frame (← or ,)">
                    <button onClick={prevFrame} className={iconBtnClass}>
                ...
                <Tooltip content="Next Frame (→ or .)">
                    <button onClick={nextFrame} className={iconBtnClass}>
                        <StepForward size={14} strokeWidth={2} />
                    </button>
                </Tooltip>
                <Tooltip content="Stop">
                    <button onClick={onStop} className={iconBtnClass}>
