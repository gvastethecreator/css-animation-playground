...
const EasingEditor = memo(
  ({ easing, onEasingChange, color, onClose, anchorEl }: EasingEditorProps) => {
        ...
        {activeTab === "presets" && (
          <div className="space-y-1 p-1 overflow-y-auto" style={{ maxHeight: "20rem" }}>
            {Object.entries(easingGroups).map(([groupName, easings]) => (
              <div key={groupName}>
                <h5 className="text-zinc-500 text-[10px] font-bold uppercase px-1 mt-1.5 first:mt-0">
                  {groupName}
                </h5>
                {easings.map((e) => (
                  <button
