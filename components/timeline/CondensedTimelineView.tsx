...
import { AnimationData, PROPERTY_COLORS, Keyframe, EasingName } from '../../types';
...
const CondensedTimelineView: React.FC<CondensedTimelineViewProps> = ({ animationData, duration, currentTime, isPlaying, easing, onCurrentTimeChange, fps }) => {
  ...
  return (
    <div 
      ref={containerRef} 
      className="relative w-full h-[14px] bg-zinc-800 rounded-full overflow-hidden cursor-ew-resize"
      onMouseDown={handleMouseDown}
    >
      {/* Easing Curve Background */}
      <div className="absolute inset-0 opacity-60">
        {containerWidth > 0 && (
           <EasingCurve easing={easing} width={containerWidth} height={14} color="#818cf8" strokeWidth={3} />
        )}
      </div>
      
      {/* Keyframe markers */}
      {Object.entries(animationData).map(([prop, track]) =>
        Array.isArray(track) && (track as Keyframe[]).map(kf => (
          <div
            key={kf.id}
