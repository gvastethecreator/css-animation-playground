...
const BezierEditor = memo(({ easing, onChange, color }: BezierEditorProps) => {
  ...
  useEffect(() => {
    ...
    const handleMouseUp = () => {
      onChange(`cubic-bezier(${p1Ref.current.x.toFixed(3)}, ${p1Ref.current.y.toFixed(3)}, ${p2Ref.current.x.toFixed(3)}, ${p2Ref.current.y.toFixed(3)})`);
      setDragging(null);
    };
  ...
  return (
    <div className="flex flex-col items-center">
      <svg ref={svgRef} width={size} height={size} className="bg-zinc-800 rounded-md cursor-pointer">
        <path d={`M ${padding} ${padding} L ${padding} ${size - padding} L ${size - padding} ${size - padding}`} fill="none" stroke="#4b5563" strokeWidth="1" />
        ...
        <circle cx={p1_svg.x} cy={p1_svg.y} r="6" fill="white" onMouseDown={handleMouseDown('p1')} className="cursor-grab active:cursor-grabbing"/>
        <circle cx={p2_svg.x} cy={p2_svg.y} r="6" fill="white" onMouseDown={handleMouseDown('p2')} className="cursor-grab active:cursor-grabbing"/>
      </svg>
