...
import { TransformState, AnimationData, TrackControlState, PROPERTY_ICONS, PROPERTY_COLORS, Keyframe, EasingName } from '../types';
...
const getUnit = (key: keyof TransformState): string => {
    ...
    if (key.includes('translate') || key.includes('blur') || key.includes('fontSize') || key.includes('letterSpacing') || key.includes('dropShadowBlur') || key.includes('dropShadowX') || key.includes('dropShadowY') || key.includes('borderRadius')) return 'px';
...
export default function LiveAnimationInfo({ isPlaying, transforms, animationData, currentTime, duration, trackControls, globalEasing }: LiveAnimationInfoProps) {
            ...
            <div className="grid grid-cols-[auto_1fr_auto_auto] gap-x-3 gap-y-1.5 items-center">
                {activeAnimatedProperties.map(prop => {
                    ...
                    if (typeof value === 'number') {
                        valueDisplay = <>{value.toFixed(precision)}{unit}</>;
                    } else if (typeof value === 'string' && prop === 'dropShadowColor') {
                        valueDisplay = <div className="w-4 h-4 rounded-sm border border-zinc-600" style={{ backgroundColor: value }} />;
                    } else {
