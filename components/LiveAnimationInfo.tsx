...
export default function LiveAnimationInfo({
  ...
  const findCurrentEasing = (track: Keyframe[], time: number): string => {
    ...
    const currentKeyframe =
      track[currentSegmentIndex >= 0 ? currentSegmentIndex : track.length - 1];
  ...
  return (
    <div className="absolute top-36 right-4 bg-zinc-950/70 backdrop-blur-sm rounded-lg p-3 text-[13px] shadow-xl text-zinc-400 font-mono z-20 pointer-events-none max-w-xs">
        ...
        {activeAnimatedProperties.map((prop) => {
          ...
          return (
            <React.Fragment key={prop}>
              ...
              <div
                className="w-6 h-6 flex items-center justify-center opacity-75"
                title={`Easing: ${easing}`}
              >
      ...
      <div className="mt-2 pt-2 border-t border-zinc-700/50">
        <h4 className="text-[11px] uppercase tracking-wider font-bold text-zinc-300">
          Global Easing
        </h4>
