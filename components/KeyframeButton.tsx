...
export default function KeyframeButton({
  ...
  const title = hasKeyframeAtCurrentTime
    ? "Remove keyframe at this time"
  ...
  const style: React.CSSProperties = {
    ...
    ["--keyframe-button-color" as string]: color,
  };
  ...
  return (
    <Tooltip content={title}>
      <button
        onClick={onClick}
        className="flex items-center justify-center w-4 h-4 rounded-full group"
      >
        <div
          className={`${baseClasses} group-hover:bg-[var(--keyframe-button-color)]`}
          style={style}
        />
      </button>
    </Tooltip>
